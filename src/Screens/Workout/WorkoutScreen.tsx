...
const WorkoutScreen = ({ route, navigation }: Props) => {
  ...
  const { exercisesCompleted, categoryExercise: { exercises } } = useAppSelector(
    (store: storeInterface) => store.fit,
  );

  const isCompletedExercises: boolean = exercisesCompleted[id].indexTo === exercises.length && exercisesCompleted[id].completed.length === excersises.length;
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      {isCompletedExercises ? <TouchableOpacity
        onPress={() => {
          dispatch(setResetExercise({ type: exercisesCompleted[id].type as TTypeExercise }));
        }}
        ...
        onPress={() => {
          if (exercisesCompleted[id].type === name) {
            dispatch(setInitialIndexScreen(Number(exercisesCompleted[id].indexTo)));
          }
